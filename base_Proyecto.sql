...
INSERT INTO `cita_inscripcion` VALUES (8,'2023-06-04','19:48:06.000000',8);
...
INSERT INTO `cupo_credito` VALUES (5,0,0,30,0,8);
...
INSERT INTO `historial_academico` VALUES (8,0,0,0,0,2,3576);
...
INSERT INTO `inscripcion_cancelacion` VALUES (16,30,20232,8);
...
INSERT INTO `resumen_credito` VALUES (11,185,0,0,0,0,8);
...
/*!50001 VIEW `inscripcion` AS select `inscripcion_cancelacion`.`Semestre` AS `Semestre`,`programa`.`Nombre` AS `Nombre`,`inscripcion_cancelacion`.`Cita_id` AS `idcita`,`facultad`.`Nombre` AS `nom` from ((((`inscripcion_cancelacion` join `cita_inscripcion` on((`cita_inscripcion`.`Id_cita` = `inscripcion_cancelacion`.`Cita_id`))) join `historial_academico` on((`historial_academico`.`Id_Historial` = `cita_inscripcion`.`Id_cita`))) join `programa` on((`programa`.`Codigo_SNIES` = `historial_academico`.`Programa_id`))) join `facultad` on((`facultad`.`Id_Facultad` = `programa`.`Facultad_id`))) */;
...
-- Dump completed on 2023-06-04 15:42:59
